...
<div id="hs-custom-backdrop-modal" class="hs-overlay hs-overlay-backdrop-open:bg-black/80 hidden size-full fixed top-0 start-0 z-[50] overflow-x-hidden overflow-y-auto pointer-events-none">
    <div class="hs-overlay-open:mt-7 z-50 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
        <div class="flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto dark:bg-neutral-800 dark:border-neutral-700 dark:shadow-neutral-700/70">
            ...
            <form action="{{ route('admin.bookings.store') }}" method="POST" class="w-full p-4 overflow-y-auto flex flex-col gap-2 justify-start items-start">
                @csrf
                <div class="w-full">
                    <label for="user_id" class="block text-sm text-slate-700 font-medium mb-2 dark:text-white">
                        User
                    </label>
                    <select name="user_id" id="user_id" class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-primary-500 focus:ring-primary-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
